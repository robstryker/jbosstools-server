...
public class QueryContribution {
	...
	protected void addListener() {
		navigator.getFilterText().addModifyListener(new ModifyListener() {
			public void modifyText(ModifyEvent e) {
				...
				final String old = oldFilterText == null ? "" : oldFilterText; //$NON-NLS-1$
				final String neww = filterText == null ? "" : filterText; //$NON-NLS-1$

				if( old.equals("") || neww.equals("") || !neww.startsWith(old)) { //$NON-NLS-1$ //$NON-NLS-2$
					clearCache();
	...
	public boolean shouldShow(Object element, Object parentElement) {
		...
		if( filterText != null && !("".equals(filterText))) { //$NON-NLS-1$
